...
class Content(db.Model):
    ...
    subject = db.Column(db.String(120))     # disciplina
    grade = db.Column(db.String(60))        # série / turma
    kind = db.Column(db.String(40))         # slide, pdf, mapa, video, imagem, plano, exercicio, resumo
    tags = db.Column(db.String(255))        # csv
...
class Exam(db.Model):
    ...
    layout = db.Column(db.String(10), default="single")  # "single" | "double"
    # questions JSON:
    # [{ "type": "objective|tf|subjective", "statement": "...",
    #    "options": ["a","b","c","d"], "correct": "texto da correta" }]
    questions_json = db.Column(db.Text, nullable=False, default="[]")
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    author = db.relationship("User", backref="exams")
    versions = db.relationship("ExamVersion", backref="exam", cascade="all, delete-orphan")
...
class ExamVersion(db.Model):
    ...
    label = db.Column(db.String(8))   # A, B, C...
    # ordered questions with shuffled options:
    # [{ "type":..., "statement":..., "options":[...], "correct_index": 2, "correct_text": "..." }]
    payload_json = db.Column(db.Text, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    attempts = db.relationship("Attempt", backref="version", cascade="all, delete-orphan")
...
class Attempt(db.Model):
    ...
    answers_json = db.Column(db.Text)     # respostas marcadas pelo aluno (índices)
    score = db.Column(db.Float)           # 0..10
